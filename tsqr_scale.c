...
// Prototype for merging the Q matrices update function.
// blockRows is the number of rows in the Q block (local Q),
...
// Frobenius norm difference between matrices A and B.
...
int qr_factor(double *A, int m, int n) {
    int lda = n;  // for economy size Q, storage is m x n, lead dimension lda
...
/*
  Check orthogonality of a matrix Q (of size m x n) by computing the Frobenius norm of (Q^T Q - I). Ideally << 1
*/
...
/*
  ...
  Then merge R factors from two blocks (each dimy x dimy) by stacking them
  to form a (2*dimy x dimy) matrix and performing an economy QR.
  Final Q is assembled from updated local Q’s.
  
  In this modified version, we loop over several thread counts (for strong and weak scaling).
*/
int main(void) {
    // Define a list of thread counts to test.
    int thread_counts[] = {1, 2, 4, 8};
    int num_tests = sizeof(thread_counts) / sizeof(thread_counts[0]);

    // Ask the user for the base matrix dimensions and scaling mode.
    // For scaling mode, use 0 for strong scaling (fixed problem size)
    // and 1 for weak scaling (problem size increases with thread count).
    int passedx, dimy, scaling_mode;
    printf("Enter base matrix dimensions (rows columns) and scaling mode (0 = strong, 1 = weak):\n");
    scanf("%d %d %d", &passedx, &dimy, &scaling_mode);

    // Loop over the thread counts.
    for (int t = 0; t < num_tests; t++) {
        int num_threads = thread_counts[t];
        omp_set_num_threads(num_threads);
        // For weak scaling, increase the number of rows with num_threads.
        int dimx = (scaling_mode == 1) ? (passedx * num_threads / 4) : passedx;
        if (dimx % 4 != 0) {
            printf("Row dimension must be divisible by 4! (dimx=%d)\n", dimx);
            continue;
        }
        int block_size = dimx / 4;
        printf("\n--- Test with %d threads, matrix size: %d x %d ---\n", num_threads, dimx, dimy);

        // Allocate the full matrix A (dimx x dimy)
        ...
        for (int b = 0; b < 4; b++){
            double *blockA = A + b * block_size * dimy; // pointer to block b
        ...
        // --- Final Merge ---
        double *Q02 = (double*) malloc(2 * dimy * dimy * sizeof(double));
        double *R02 = (double*) malloc(dimy * dimy * sizeof(double));
        if (!Q02 || !R02) {
            printf("Allocation failed for final merge.\n");
            return 1;
        }
        merge_R(R01, R23, dimy, Q02, R02);
        // Stack Q0 and Q1 vertically and similarly Q2 and Q3.
        ...
        double exec_time = tsqr_end - tsqr_start;
        printf("\nTSQR execution time with %d threads: %f seconds\n", num_threads, exec_time);
        ...
        // --- Direct Full QR Factorization for Reference ---
        ...
        // --- Free allocated arrays for this test ---
        free(A);
